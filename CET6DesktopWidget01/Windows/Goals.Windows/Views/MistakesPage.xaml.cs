...
public partial class MistakesPage : UserControl
{
    ...
    private void Refresh()
    {
        ...
        var words = _vm.GetSpecialWords(_mode == "banished", q).ToList();
        List.ItemsSource = words; Status.Text = _mode == "banished" ? $"{words.Count} 个已斩词 · 点击恢复后重新进入闪卡队列" : $"{words.Count} 个收藏错词 · 再次点击可取消收藏";
    }
